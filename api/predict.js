...
async function api(path,key,ttl=0){
  ...
  // IMPORTANT:
  // Missing quota headers must remain null.
  // They must NEVER be converted to 0.
  const dailyHeader=r.headers.get("x-ratelimit-requests-remaining");
  const minuteHeader=r.headers.get("x-ratelimit-remaining");
  ...
  if(Number.isFinite(daily)){
    quotaState.daily=daily;
  }

  if(Number.isFinite(minute)){
    quotaState.minute=minute;
  }
...
function congestion(list,target,days=14){
  ...
  return list.filter(
    f=>
      new Date(f.fixture.date).getTime()>=cut
  ).length;
}
...
function hitRate(list,rule,id){
  ...
  for(const f of list.slice(0,10)){
    ...
    if(rule==="team05"&&gf>=1)h++;
    if(rule==="over15"&&t>=2)h++;
    if(rule==="over25"&&t>=3)h++;
    if(rule==="under35"&&t<=3)h++;
    if(rule==="under45"&&t<=4)h++;
    if(rule==="btts"&&gf>0&&ga>0)h++;
    if(rule==="nonloss"&&gf>=ga)h++;
    if(rule==="win"&&gf>ga)h++;
    if(rule==="draw"&&gf===ga)h++;
  }
...
function probabilitySet(lh,la){
  const m=
    dixonColesMatrix(lh,la);
  ...
  return {
    ...
    "under4.5":g.under45,
    btts:h05*a05,
    homeOrDraw:m.home+m.draw,
    awayOrDraw:m.away+m.draw,
    homeWin:m.home,
    draw:m.draw,
    awayWin:m.away
  };
...
function statValue(teamStats,type){
  const item=
    (teamStats||[])
      .find(
        x=>
...
function timingProfile(d,side){
  const m=
    d?.response?.goals?.[side]?.minute||
    {};
...
function lineupInfo(d){
  ...
  const good=
    ls.filter(
      x=>
        Array.isArray(x?.startXI) &&
        x.startXI.length>=11
    );

  return {
    confirmed:good.length>=2,
    lineups:good
  };
}

function lineupFor(info,teamId){
  return info.lineups.find(
    x=>x?.team?.id===teamId
  )||null;
}

function xiPlayers(lineup){
  return (
    lineup?.startXI||[]
  )
    .map(x=>({
...
function xiStrength(players,map){
  if(!players.length){
    return null;
  }
  ...
  vals.forEach(x=>{
    ...
    if(pos.startsWith("G")){
      ...
      groups.defence.push(x);
    }else if(pos.startsWith("M")){
      groups.midfield.push(x);
    }else{
      groups.attack.push(x);
    }
  });
  ...
  return {
    ...
    defence:+defence.toFixed(2),
    covered:vals.length,
    units:{
      gk:unit(groups.gk),
      defence:unit(groups.defence),
      midfield:unit(groups.midfield),
      attack:unit(groups.attack)
    }
  };
}
...
function externalProb(d){
  const p=
    d?.response?.[0]?.predictions?.percent||
    {};
  ...
  return {
    ...
    draw:dr/total,
    awayWin:a/total,
    homeOrDraw:(h+dr)/total,
    awayOrDraw:(a+dr)/total
  };
}

function engineAgreement(values){
  const xs=
    values.filter(
      Number.isFinite
    );
...
function plainReasons({
  ...
  if(
    ...
    out.push(
      `The match profile points to about ${total.toFixed(1)} total goals, comfortably below this line.`
    );
  }else if(
    rule==="over1.5" ||
    rule==="over2.5" ||
    ...
    out.push(
      `The attacking profile points to about ${total.toFixed(1)} total goals.`
    );
  }else if(
    rule==="home0.5"
  ){
    out.push(
      `${homeTeam}'s attacking numbers give them a strong chance of scoring at least once.`
    );
  }else if(
    rule==="away0.5"
  ){
    out.push(
      `${awayTeam}'s attacking numbers give them a strong chance of scoring at least once.`
    );
  }else{
    out.push(
      `Recent strength and home/away performance support this result outcome.`
    );
  }
  ...
  if(
    ...
    out.push(
      `Recent shot and chance pressure supports the forecast rather than relying on goals alone.`
    );
  }

  if(agreement>=85){
    out.push(
      `The independent model checks broadly agree with each other.`
    );
  }else if(agreement<70){
    out.push(
      `Some model checks disagree, so the displayed percentage has been held back.`
    );
  }
  ...
  if(
    ...
    out.push(
      `Neither side has a major recovery-time advantage.`
    );
  }
...
function watchItems({
  ...
  if(!linfo.confirmed){
    out.push(
      "Starting line-ups are not confirmed yet, so the forecast can still move."
    );
  }

  if(hasIntervening){
    out.push(
      `A team plays before this fixture, so this analysis will be rebuilt afterwards.`
    );
  }

  if(agreement<70){
    out.push(
      "The model engines disagree more than usual."
    );
  }
  ...
  try{
    ...
    if(
      ...
      return res
        .status(422)
        .json({
          error:"Not enough recent completed matches"
        });
    }
    ...
    if(
      ...
      const hCreate=
        (
          hChance.for.pressure+
          aChance.against.pressure
        )/
        2;

      const aCreate=
        (
          aChance.for.pressure+
          hChance.against.pressure
        )/
        2;
      ...
      rules.map(rule=>{
        ...
        return {
          ...
          stats:{
            ...
            homeFormation:
              hLine?.formation??null,

            awayFormation:
              aLine?.formation??null,

            homeFormationContinuity:
              hFormation===null
                ? null
                : Math.round(
                    hFormation*100
            ...
            homeChance:
              hChance.for?.pressure??null,

            awayChance:
              aChance.for?.pressure??null,

            homeSuppression:
              hChance.against?.pressure??null,

            awaySuppression:
              aChance.against?.pressure??null,

            homeSOT:
              hChance.for?.sot??null,

            awaySOT:
              aChance.for?.sot??null,
    ...
    const derived=[
...

      {
        ...
        type:"clean",
        title:
          `${b.awayTeam} clean sheet`,
        v:pAwayCS
      },

      {
        rule:
          `exact:${bestScore.h}-${bestScore.a}`,

        type:"score",

        title:
          `Exact score ${bestScore.h}-${bestScore.a}`,
    ...
    for(const d of derived){
      ...
      predictions.push({
        ...
        reasons:[
          "Derived coherently from the calibrated Match Index score distribution."
        ],
        ...
        watch:
          watchItems({
            ...
            aBefore,
            agreement:
              baseDerivedStats.engineAgreement
          }),

        technical:
          "Derived from the same expected-goals score distribution as the match-result and goals forecasts.",

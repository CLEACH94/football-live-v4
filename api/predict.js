...
const MODEL_VERSION = "v16-match-index-independent-1";
...
const cache = new Map();
let quotaState={daily:null,minute:null};
...
async function api(path,key,ttl=0){
  ...
  const daily=Number(r.headers.get("x-ratelimit-requests-remaining")),minute=Number(r.headers.get("x-ratelimit-remaining"));
  if(Number.isFinite(daily))quotaState.daily=daily;if(Number.isFinite(minute))quotaState.minute=minute;
...
function xiPlayers(lineup){return(lineup?.startXI||[]).map(x=>({id:x?.player?.id,name:x?.player?.name,pos:String(x?.player?.pos||x?.player?.position||"").toUpperCase(),grid:String(x?.player?.grid||"")})).filter(x=>x.id)}
...
function pairLinks(players,historicalIds){if(!players.length||!historicalIds.length)return[];const parseGrid=g=>{const [r,c]=String(g||"").split(":").map(Number);return Number.isFinite(r)&&Number.isFinite(c)?{r,c}:null};const out=[];for(let i=0;i<players.length;i++)for(let j=i+1;j<players.length;j++){const a=players[i],b=players[j],ga=parseGrid(a.grid),gb=parseGrid(b.grid);if(!ga||!gb)continue;const dist=Math.hypot((ga.r-gb.r)*1.35,ga.c-gb.c);if(dist>2.25)continue;const together=historicalIds.filter(x=>x.includes(a.id)&&x.includes(b.id)).length/historicalIds.length;out.push({a:a.id,b:b.id,rating:Math.round(clamp(52+together*43,52,95)),sample:historicalIds.length});}return out.sort((x,y)=>y.rating-x.rating)}
function playerDisplay(players,map,teamImpact){return players.map(p=>{const d=map?.get(p.id)||null;if(!d)return{id:p.id,name:p.name,pos:p.pos,grid:p.grid,rating:null,sourceRating:null,minutes:0,confidence:0,matchup:teamImpact};const base=num(d.rating,6.5),minutes=num(d.minutes,0),mi=Math.round(clamp(50+(base-6)*12+Math.min(8,Math.log10(minutes+10)*2.5),50,94));return{id:p.id,name:p.name,pos:p.pos,grid:p.grid,rating:mi,sourceRating:+base.toFixed(2),minutes,confidence:Math.round(clamp(45+Math.log10(minutes+10)*18,45,96)),matchup:teamImpact}})}
function benchPlayers(lineup){return(lineup?.substitutes||[]).map(x=>({id:x?.player?.id,name:x?.player?.name,pos:String(x?.player?.pos||x?.player?.position||"").toUpperCase()})).filter(x=>x.id)}
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    let inj=null,lineupD=null,h2hD=null,statResponses=[];
    if(mode==="deep"){
      [inj,lineupD,h2hD]=await Promise.all([
        api(`/injuries?fixture=${fixtureId}`,key,180000).catch(()=>null),api(`/fixtures/lineups?fixture=${fixtureId}`,key,120000).catch(()=>null),
        api(`/fixtures/headtohead?h2h=${homeId}-${awayId}&last=5`,key,600000).catch(()=>null)
      ]);
    ...
    const h2h=rows(h2hD).filter(done).slice(0,5);let h2hHomeImpact=0,h2hSample=0;if(mode==="deep"&&h2h.length>=3){let hg=0,ag=0,n=0;for(const f of h2h){const x=gfga(f,homeId);if(!x)continue;hg+=x[0];ag+=x[1];n++}if(n){h2hSample=n;const gd=(hg-ag)/n;h2hHomeImpact=Math.round(clamp(gd*1.6,-4,4));squadLH=squadLH*.98+(hg/n)*.02;squadLA=squadLA*.98+(ag/n)*.02}}
    ...
    const engineSets=engines.map(e=>({name:e.name,p:probabilitySet(clamp(e.lh,.15,3.8),clamp(e.la,.13,3.6))}));
    ...
    let q=dataQuality({stage:mode,homeN:h20.length,awayN:a20.length,homeSplitN:hHome.length,awaySplitN:aAway.length,season:!!(hs&&as),table:!!(hSt&&aSt),chance:!!(hChance.for&&aChance.for),injuries:!!inj,external:false,lineupConfirmed:linfo.confirmed,playerStrength:!!(hXi&&aXi),partnership:hContinuity!==null&&aContinuity!==null,formation:hFormation!==null&&aFormation!==null,timing:!!(hTiming&&aTiming),leagueScore:league.score});if(hasIntervening&&!linfo.confirmed)q=Math.max(54,q-8);
    ...
    const rules=Object.keys(titles),predictions=rules.map(rule=>{const values=engineSets.map(e=>e.p[rule]).filter(Number.isFinite);const mean=values.reduce((a,x)=>a+x,0)/values.length,agree=engineAgreement(values),disagreementPenalty=clamp((100-agree.score)/100*.10,0,.055),raw=clamp(mean-(mean>.5?disagreementPenalty:-disagreementPenalty*.25),.03,.97),cal=confidenceCalibrate(raw,q),hr=histRule[rule]?hitRate(...histRule[rule]):rule==="draw"?Math.round((hitRate(h20,"draw",homeId)+hitRate(a20,"draw",awayId))/2):rule==="btts"?Math.round((hitRate(h20,"btts",homeId)+hitRate(a20,"btts",awayId))/2):rule==="over1.5"?Math.round((hitRate(h20,"over15",homeId)+hitRate(a20,"over15",awayId))/2):rule==="over2.5"?Math.round((hitRate(h20,"over25",homeId)+hitRate(a20,"over25",awayId))/2):rule==="under3.5"?Math.round((hitRate(h20,"under35",homeId)+hitRate(a20,"under35",awayId))/2):Math.round((hitRate(h20,"under45",homeId)+hitRate(a20,"under45",awayId))/2);const reasons=plainReasons({rule,p:cal,q,agreement:agree.score,hChance,aChance,homeRest,awayRest,linfo,hasIntervening,homeTeam:b.homeTeam,awayTeam:b.awayTeam,finalLH,finalLA}),watch=watchItems({linfo,hasIntervening,hBefore,aBefore,agreement:agree.score});return{id:`${fixtureId}|${rule}`,type:types[rule],title:titles[rule],rule,p:Math.round(cal*100),rawP:Math.round(raw*100),reasons,watch,technical:`Match Index independent ensemble uses opponent-adjusted recent form, home/away splits, season attack/defence, Dixon–Coles score modelling, schedule/rest context${mode==="deep"?", chance creation, defensive suppression and availability":""}${linfo.confirmed?", confirmed XI unit strength, formation and partnership continuity":""}.`,stats:{expectedHome:+finalLH.toFixed(2),expectedAway:+finalLA.toFixed(2),homePosition:hSt?.pos??null,awayPosition:aSt?.pos??null,dataConfidence:q,last5HitRate:hr,homeForm:form(h20,homeId),awayForm:form(a20,awayId),h2hGames:h2h.length,homeSample:h20.length,awaySample:a20.length,homeVenueSample:hHome.length,awayVenueSample:aAway.length,lineupConfirmed:linfo.confirmed,homeXI:hXi?.rating??null,awayXI:aXi?.rating??null,homeUnits:hXi?.units??null,awayUnits:aXi?.units??null,homeContinuity:hContinuity===null?null:Math.round(hContinuity*100),awayContinuity:aContinuity===null?null:Math.round(aContinuity*100),homeFormation:hLine?.formation??null,awayFormation:aLine?.formation??null,homeFormationContinuity:hFormation===null?null:Math.round(hFormation*100),awayFormationContinuity:aFormation===null?null:Math.round(aFormation*100),homeChance:hChance.for?.pressure??null,awayChance:aChance.for?.pressure??null,homeSuppression:hChance.against?.pressure??null,awaySuppression:aChance.against?.pressure??null,homeSOT:hChance.for?.sot??null,awaySOT:aChance.for?.sot??null,homeInjuries:hInj,awayInjuries:aInj,homeRestDays:homeRest,awayRestDays:awayRest,homeCongestion14:homeCongestion,awayCongestion14:awayCongestion,homePlaysBefore:hBefore.length>0,awayPlaysBefore:aBefore.length>0,interveningHome:hBefore[0]?`${hBefore[0].teams.home.name} v ${hBefore[0].teams.away.name}`:null,interveningAway:aBefore[0]?`${aBefore[0].teams.home.name} v ${aBefore[0].teams.away.name}`:null,engineAgreement:agree.score,engineSpread:agree.spread,engineCount:agree.count,externalModel:false,engineProbabilities:Object.fromEntries(engineSets.map(e=>[e.name,Math.round((e.p[rule]??0)*100)])),ablation:Object.fromEntries(values.map((_,i)=>{const xs=values.filter((__,j)=>j!==i);return[`without${i+1}`,xs.length?Math.round(xs.reduce((a,x)=>a+x,0)/xs.length*100):null]})),analysisStage:linfo.confirmed?"final":mode,freshness:linfo.confirmed?100:hasIntervening?62:Math.round(clamp(80+(q-70)*.4,70,96))}}});
    // Coherent derived outputs from the same score distribution.
    const pHome15=1-Math.exp(-finalLH)*(1+finalLH),pAway15=1-Math.exp(-finalLA)*(1+finalLA);
    const pHomeCS=Math.exp(-finalLA),pAwayCS=Math.exp(-finalLH);
    let bestScore={h:0,a:0,p:0};for(let h=0;h<=6;h++)for(let a=0;a<=6;a++){const sp=pois(h,finalLH)*pois(a,finalLA);if(sp>bestScore.p)bestScore={h,a,p:sp}}
    const baseDerivedStats={expectedHome:+finalLH.toFixed(2),expectedAway:+finalLA.toFixed(2),dataConfidence:q,lineupConfirmed:linfo.confirmed,engineAgreement:engineAgreement(engineSets.map(e=>e.p.homeWin)).score,analysisStage:linfo.confirmed?"final":mode,freshness:linfo.confirmed?100:Math.round(clamp(80+(q-70)*.4,70,96))};
    const derived=[
      {rule:"home1.5",type:"teamgoal",title:`${b.homeTeam} over 1.5 goals`,v:pHome15},
      {rule:"away1.5",type:"teamgoal",title:`${b.awayTeam} over 1.5 goals`,v:pAway15},
      {rule:"homeCleanSheet",type:"clean",title:`${b.homeTeam} clean sheet`,v:pHomeCS},
      {rule:"awayCleanSheet",type:"clean",title:`${b.awayTeam} clean sheet`,v:pAwayCS},
      {rule:`exact:${bestScore.h}-${bestScore.a}`,type:"score",title:`Exact score ${bestScore.h}-${bestScore.a}`,v:bestScore.p}
    ];
    for(const d of derived){const cp=confidenceCalibrate(clamp(d.v,.01,.99),q);predictions.push({id:`${fixtureId}|${d.rule}`,type:d.type,title:d.title,rule:d.rule,p:Math.round(cp*100),rawP:Math.round(d.v*100),reasons:[`Derived coherently from the calibrated Match Index score distribution.`],watch:watchItems({linfo,hasIntervening,hBefore,aBefore,agreement:baseDerivedStats.engineAgreement}),technical:"Derived from the same expected-goals score distribution as the match-result and goals forecasts.",stats:{...baseDerivedStats,last5HitRate:null}})}

    const byRule=Object.fromEntries(predictions.map(x=>[x.rule,x]));
    const resultRules=["homeWin","draw","awayWin"],resultSum=resultRules.reduce((a,r)=>a+(byRule[r]?.p||0),0)||100;resultRules.forEach(r=>{if(byRule[r])byRule[r].p=Math.round((byRule[r].p/resultSum)*100)});const normSum=resultRules.reduce((a,r)=>a+(byRule[r]?.p||0),0),diff=100-normSum;if(diff&&byRule.homeWin)byRule.homeWin.p+=diff;
    if(byRule.homeOrDraw&&byRule.homeWin&&byRule.draw)byRule.homeOrDraw.p=byRule.homeWin.p+byRule.draw.p;if(byRule.awayOrDraw&&byRule.awayWin&&byRule.draw)byRule.awayOrDraw.p=byRule.awayWin.p+byRule.draw.p;
    if(byRule["over1.5"]&&byRule["over2.5"]&&byRule["over2.5"].p>byRule["over1.5"].p)byRule["over2.5"].p=byRule["over1.5"].p;if(byRule["under3.5"]&&byRule["under4.5"]&&byRule["under3.5"].p>byRule["under4.5"].p)byRule["under3.5"].p=byRule["under4.5"].p;
    predictions.sort((a,b)=>b.p-a.p);
    const lineupPayload=linfo.confirmed?{home:{teamId:homeId,team:b.homeTeam,formation:hLine?.formation||null,coach:hLine?.coach?.name||null,xi:playerDisplay(hPlayers,pmap,h2hHomeImpact),bench:benchPlayers(hLine),partnerships:hLinks,continuity:hContinuity===null?null:Math.round(hContinuity*100),formationContinuity:hFormation===null?null:Math.round(hFormation*100),matchupImpact:h2hHomeImpact,matchupSample:h2hSample},away:{teamId:awayId,team:b.awayTeam,formation:aLine?.formation||null,coach:aLine?.coach?.name||null,xi:playerDisplay(aPlayers,pmap,-h2hHomeImpact),bench:benchPlayers(aLine),partnerships:aLinks,continuity:aContinuity===null?null:Math.round(aContinuity*100),formationContinuity:aFormation===null?null:Math.round(aFormation*100),matchupImpact:-h2hHomeImpact,matchupSample:h2hSample}}:null;
    return res.status(200).json({model:MODEL_VERSION,source:"API-Football",quota:quotaState,fixtureId,analysisStage:linfo.confirmed?"final":mode,lineupConfirmed:linfo.confirmed,lineupChecked:mode==="deep",hasIntervening,interveningUntil,scheduleContext:{homeRestDays:homeRest,awayRestDays:awayRest,homeCongestion14:homeCongestion,awayCongestion14:awayCongestion,homePlaysBefore:hBefore.length>0,awayPlaysBefore:aBefore.length>0},evidence:{opponentAdjusted:true,chanceStats:!!(hChance.for&&aChance.for),defensiveSuppression:!!(hChance.against&&aChance.against),externalModel:false,playerStrength:!!(hXi&&aXi),partnerships:hContinuity!==null&&aContinuity!==null,formation:hFormation!==null&&aFormation!==null,timing:!!(hTiming&&aTiming),scheduleContext:true},dataLayers:{teamData:q,squadData:linfo.confirmed?(hXi&&aXi?95:70):35,contextData:Math.round(clamp(70+(hChance.for?10:0)+(hTiming&&aTiming?5:0),50,98)),overall:q},lineups:lineupPayload,predictions});
